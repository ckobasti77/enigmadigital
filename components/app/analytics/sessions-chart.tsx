...
/**
 * Sessions + conversions over time. Two measures of different scale share the
 * x-axis as stacked panels (never a dual y-axis): sessions as an area on top,
 * conversions as bars beneath. Each panel is titled directly — no legend.
 */

const config = {
  sessions: { label: "Sesije", color: "var(--color-chart-1)" },
  conversions: { label: "Konverzije", color: "var(--color-chart-2)" },
} satisfies ChartConfig;

const SYNC_ID = "ga4-timeline";
const TOP_H = "h-52";
const BOTTOM_H = "h-24";
const Y_WIDTH = 44;

/** Round ticks (0, step, 2·step, …) that cover `max` — never 95/190/285. */
...
function tickFormatterFor(count: number) {
  // ≤ 7 x labels regardless of range length.
...
export function SessionsChart({ data }: { data: DailyPoint[] }) {
  ...
  const sessionTicks = niceTicks(Math.max(...data.map((d) => d.sessions)), 3);
  const conversionTicks = niceTicks(
    Math.max(...data.map((d) => d.conversions)),
    1,
  );
  ...
  return (
    <Card className="gap-0 py-0 shadow-card ring-line">
      <div className="px-5 pt-5">
        <PanelTitle color="var(--color-chart-1)" title="Sesije" />
      </div>
      <ChartContainer
        config={config}
      ...
      >
        ...
        >
          <defs>
            <linearGradient id="ga4-sessions-fill" x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="0%"
          ...
          <YAxis
            ...
            ticks={sessionTicks}
            interval={0}
            domain={[0, sessionTicks[sessionTicks.length - 1]]}
      ...
      <div className="mt-3 border-t border-line-soft px-5 pt-4">
        <PanelTitle color="var(--color-chart-2)" title="Konverzije" />
      </div>
      <ChartContainer
        config={config}
      ...
      >
        ...
        >
          ...
          <YAxis
            ...
            axisLine={false}
            tickLine={false}
            ticks={conversionTicks}
            interval={0}
            domain={[0, conversionTicks[conversionTicks.length - 1]]}
            allowDecimals={false}
            tickFormatter={(v: number) => formatNumber(v)}
            tick={{ fill: "var(--color-text-muted)", fontSize: 11 }}
          />
    ...
    <div className="min-w-44 rounded-lg border border-line bg-popover px-3 py-2 text-xs shadow-card">
      ...
      <dl className="mt-1.5 space-y-1">
        <TooltipRow
          ...
          label="Sesije"
          value={formatNumber(point.sessions)}
        />
        <TooltipRow
          color="var(--color-chart-2)"
          label="Konverzije"
          value={formatNumber(point.conversions)}
        />
      </dl>
    </div>
  );
