...
export type FbCommentThreadData = FunctionReturnType<
  typeof api.fbCommentsStore.listThreads
>[number];

type FbCommentReply = FbCommentThreadData["replies"][number];

/** Facebook rejects a longer comment, and says so at the edge. */
 ...
 *
 * Identical vocabulary to the Instagram screen on purpose: cyan means "your
 * turn", amber means hidden, red means gone. Somebody who works both queues
 * learns the rail once.
 */
function railClass(thread: FbCommentThreadData): string {
...
/**
 * Stand-in for a profile picture. Facebook hands out a display name and no
 * avatar on this edge, so the initial of the name does the identifying rather
 * than a grey circle that looks like a failed image.
 */
function Avatar({ name, ours }: { name: string; ours: boolean }) {
  ...
  return (
    <span className="flex min-w-0 flex-wrap items-center gap-1.5">
      <span className="truncate text-sm font-semibold text-foreground">
        {name || (ours ? "Naša stranica" : "Nepoznat nalog")}
      </span>
      {ours && (
...
function ReplyComposer({
  ...
  const reply = useAction(api.fbComments.replyToComment);
 ...
 *
 * One more than the Instagram screen has: „Lajkuj”. Facebook exposes
 * `POST /{object-id}/likes`, Instagram exposes nothing of the kind at any
 * permission level — so this is not a control that was forgotten over there.
 */
function CommentActions({
  ...
  const setHidden = useAction(api.fbComments.setCommentHidden);
  const setLiked = useAction(api.fbComments.setCommentLiked);
  const remove = useAction(api.fbComments.deleteComment);

  const hiddenFlag = useOptimisticToggle(hidden);
  const likedFlag = useOptimisticToggle(liked);
  ...
  const toggleHidden = async () => {
    const next = !hiddenFlag.value;
    onError(null);
    try {
      await hiddenFlag.run(next, () => setHidden({ commentId, hidden: next }));
    } catch (err) {
      onError(
        convexMessage(
          err,
  ...
  const toggleLiked = async () => {
    const next = !likedFlag.value;
    onError(null);
    try {
      await likedFlag.run(next, () => setLiked({ commentId, liked: next }));
    } catch (err) {
      onError(
        convexMessage(
          err,
    ...
    <>
      <div className="mt-2 flex flex-wrap items-center gap-1">
        ...
        <Button
          ...
          variant="ghost"
          size="xs"
          aria-pressed={likedFlag.value}
          // A second press before the first answers would be two opposite
          // writes to Facebook, settled by whichever finished last (V2/3).
          disabled={likedFlag.pending}
          onClick={() => void toggleLiked()}
          className={cn(
            "text-text-muted hover:text-foreground",
            likedFlag.value && "text-accent-400 hover:text-accent-400",
          )}
        >
          <ThumbsUp data-icon="inline-start" />
          {likedFlag.value ? "Lajkovano" : "Lajkuj"}
        </Button>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            ...
            Brisanje je trajno. Komentar nestaje sa Facebook-a i ne može da se
 ...
 */
export function FbCommentThread({
  thread,
  selected,
  onSelectedChange,
  showPost = false,
  ...
  const permalink = thread.permalink ?? thread.post?.permalink;
      ...
      <div className="flex gap-3">
        {selectable && (
          <Checkbox
            ...
            aria-label={`Izaberi komentar korisnika ${thread.authorName || "bez imena"}`}
            className="mt-1.5"
          />
        )}

        <Avatar name={thread.authorName} ours={thread.isOurs} />
            ...
            {showPost && (
              <span className="inline-flex max-w-full items-center gap-1 rounded bg-surface-raised px-1.5 py-0.5 text-micro text-text-muted">
                <span className="font-semibold uppercase tracking-wider">
                  {thread.post?.statusType ?? "OBJAVA"}
                </span>
                <span className="truncate">
                  {thread.post?.message?.trim()
                    ? thread.post.message
                    : "bez opisa"}
                </span>
              </span>
          ...
          {thread.replies.length > 0 && (
            <div className="mt-3 flex flex-col gap-3">
              {thread.replies.map((reply) => (
                <ReplyRow key={reply._id} reply={reply} onError={setError} />
              ))}
            </div>
          )}
        </div>
