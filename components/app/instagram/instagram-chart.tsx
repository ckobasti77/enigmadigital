...
/**
 * Followers count + daily reach over time. Two measures of different scale
 * share the x-axis as stacked panels (never a dual y-axis): followers count
 * as an area on top, daily reach as bars beneath.
 */

const config = {
  followersCount: { label: "Pratioci", color: "var(--color-chart-1)" },
  reach: { label: "Doseg", color: "var(--color-chart-2)" },
} satisfies ChartConfig;

const SYNC_ID = "instagram-timeline";
const TOP_H = "h-52";
const BOTTOM_H = "h-24";
const Y_WIDTH = 48;
...
export function InstagramChart({ data }: { data: IgDailyPoint[] }) {
  ...
  const maxFollowers = Math.max(...data.map((d) => d.followersCount), 0);
  const followerTicks = niceTicks(maxFollowers, 3);
  const reachTicks = niceTicks(
    Math.max(...data.map((d) => d.reach), 0),
    1,
  );
  ...
  return (
    <Card className="gap-0 py-0 shadow-card ring-line">
      <div className="px-5 pt-5">
        <PanelTitle color="var(--color-chart-1)" title="Broj pratilaca" />
      </div>
      <ChartContainer
        config={config}
      ...
      >
        ...
        >
          <defs>
            <linearGradient id="ig-followers-fill" x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="0%"
                stopColor="var(--color-followersCount)"
                stopOpacity={0.22}
              />
              <stop
                offset="100%"
          ...
          <YAxis
            ...
            ticks={followerTicks}
            interval={0}
            domain={[0, followerTicks[followerTicks.length - 1]]}
          ...
          <Area
            ...
            type="monotone"
            stroke="var(--color-followersCount)"
            strokeWidth={2}
            fill="url(#ig-followers-fill)"
            dot={false}
            activeDot={{
              r: 4,
              strokeWidth: 2,
              stroke: "var(--color-card)",
              fill: "var(--color-followersCount)",
            }}
            isAnimationActive={false}
          />
        </AreaChart>
      ...
      <div className="mt-3 border-t border-line-soft px-5 pt-4">
        <PanelTitle color="var(--color-chart-2)" title="Dnevni doseg (Reach)" />
      </div>
      <ChartContainer
        config={config}
      ...
      >
        ...
        >
          ...
          <YAxis
            ...
            ticks={reachTicks}
            interval={0}
            domain={[0, reachTicks[reachTicks.length - 1]]}
            allowDecimals={false}
            tickFormatter={(v: number) => formatNumber(v)}
            tick={{ fill: "var(--color-text-muted)", fontSize: 11 }}
          />
...
function TimelineTooltip({
  ...
  active?: boolean;
  payload?: Array<{ payload?: IgDailyPoint }>;
}) {
    ...
    <div className="min-w-44 rounded-lg border border-line bg-popover px-3 py-2 text-xs shadow-card">
      ...
      <dl className="mt-1.5 space-y-1">
        <TooltipRow
          ...
          label="Pratioci"
          value={formatNumber(point.followersCount)}
        />
        <TooltipRow
          color="var(--color-chart-2)"
          label="Doseg"
          value={formatNumber(point.reach)}
        />
      </dl>
    </div>
  );
