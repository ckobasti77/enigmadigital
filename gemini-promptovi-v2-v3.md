# ENIGMA COMMAND CENTER — V2 + V3 promptovi za Antigravity (Gemini 3.7 Flash · Effort: HIGH za sve)

Pravila: svaki prompt u NOVU konverzaciju · `git status` čist pre svakog · posle svakog javi Claudeu "završio GX" za review · commit poruka je data uz svaki prompt.

---

## G7 (P12) — Meta Ads hijerarhija + insights sync
Commit posle: `P12: Meta Ads hierarchy + insights sync (awaiting System User token)`
...
## G8 (P13) — Ads ekrani: Campaigns pregled + drill-down
Commit posle: `P13: Ads screens - campaigns + drill-down`
...
## G9 (P14) — Hook Battle ekran
Commit posle: `P14: Hook Battle screen`
...
3. Statistical honesty: per version an evidence meter; below thresholds (default 1000 impressions / 50 clicks, configurable) show "rano za zaključak" and DO NOT crown a leader; info popover explains why.
...
Commit posle: `P16: rules engine + notifications`
...
VERIFICATION: seed historical adInsights where one rule's condition is true and another is true but below minImpressions -> run evaluator via throwaway trigger: first fires exactly once (records firing, honors cooldown on second run), second NEVER fires; email send path with RESEND_API_KEY missing -> logs cleanly, no crash; clean up seeds. npx tsc --noEmit && npm run lint && npm run build must pass. List files + summary.
```

---

## G12 (P17) — Google Ads read integracija
Commit posle: `P17: Google Ads read integration (awaiting approval)`
...
## G13 (P18) — „Duplicate with new hook" iz dashboarda
Commit posle: `P18: duplicate-with-new-hook flow`
...
## P11 — Pixel + Conversions API (⚠ SAJT repo `enigmait`, NE command center!)
Commit posle (u sajt repou): `Meta Pixel + CAPI (Lead dedup, awaiting env)`
